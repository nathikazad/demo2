...
type Digraph interface {
  GetVerts() map[uint]*Vertex  // map vertex ID to vertex reference
  GetEdges() map[uint]*Edge  // map edge ID to edge reference
}

// ShortestPath - solve for the shortest deighted directional path from start to end vertex.
func ShortestPath(g *Digraph, startVertID, endVertID uint) (edgeIDs []uint, dist float64) {
